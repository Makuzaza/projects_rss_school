document.addEventListener("DOMContentLoaded", () => {
      ...
      bestGifts.forEach((gift) => {
        ...
        giftDiv.addEventListener("click", () => {
          ...
          modalDescription.innerText =
            gift.description || "No additional description available.";
  ...
  function getMoveAmount() {
    ...
    const visibleArea = sliderContainer.offsetWidth;
    const screenWidth = window.innerWidth;

    // Subtract margins for screens larger than 780px
    const adjustedWidth =
      screenWidth > 780 ? totalWidth + 126 : totalWidth + 10;
  ...
  function updateSliderPosition() {
    const visibleArea = sliderContainer.offsetWidth; // Recalculate visible area dynamically
    const moveAmount = getMoveAmount();

    // Prevent scrolling beyond bounds
    const maxPosition = Math.ceil(
      (featuresContainer.scrollWidth - visibleArea) / moveAmount
    );
    ...
    featuresContainer.style.transform = `translateX(-${
  ...
  function updateCountdown() {
    ...
    if (difference > 0) {
      ...
      document.querySelector(
        ".countdown .countdown-item:nth-of-type(1) .time"
      ).textContent = days;
      document.querySelector(
        ".countdown .countdown-item:nth-of-type(2) .time"
      ).textContent = hours;
      document.querySelector(
        ".countdown .countdown-item:nth-of-type(3) .time"
      ).textContent = minutes;
      document.querySelector(
        ".countdown .countdown-item:nth-of-type(4) .time"
      ).textContent = seconds;
    } else {
